...
struct RegistrationOTPView: View {
    ...
    func validateOTP() {
        let newUser = RegisterNewUserRequest(firstName: viewModel.firstName, lastName: viewModel.lastName, title: viewModel.title, gender: viewModel.gender, emailAddress: viewModel.email, password: viewModel.password, mobile: viewModel.mobile, dob: viewModel.dob.getFormattedDate(format: Constants.DateFormats.DOB_DATE_FORMAT_FOR_SERVER), os: Constants.Device.OS, token: "", deviceType: "[\(UIDevice.current.systemVersion), \(UIDevice.modelName)]", appVer: Bundle.main.releaseVersionNumber.asStringOrNilText(), apiKey: UserDefaults.jwtKey, sportId: viewModel.selectedSportId!, stateId: viewModel.states[viewModel.selectedStateIndex].id, districtId: viewModel.districts[viewModel.selectedDistrictIndex].id)
